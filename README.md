...
- **主题系统** —— 内置五款风格各异的主题:「素构 · 浅色」「素构 · 暗色」(侧栏布局)、「墨阅」(报刊衬线风)、「终端」(命令行风,顶部导航)、「画廊」(卡片现代风,顶部导航),移动端自适应(侧栏主题折叠为抽屉面板,顶栏主题横向导航,文章较多时导航可横向滚动、目录全宽下拉,任何屏宽都能看到全部文章),目录支持折叠(跨页记忆展开状态),页面过渡动画预设,可选「由 Plainstruct 创建」底部署名;可视化配置面板(颜色/字号/选项/开关);主题制作器(代码编辑 + 实时预览);主题以 ZIP 包导入导出
